...
The `cols_merge_n_pct()` function wraps the percentage column's value in parentheses and places it after the count, but it relies on you to format the percentage column beforehand with `fmt_percent()` (or another formatter). Without that step, the raw numeric values appear unformatted in the merged cell.
